 ...
 */
export function createBugReport(data: NewBugReport): BugReport {
 ...
 */
export function listBugReports(status?: BugStatus): BugReport[] {
  if (status) {
    return db.prepare(`
      SELECT * FROM bug_reports
      WHERE status = ?
      ORDER BY created_at DESC
    `).all(status) as BugReport[];
  }
 ...
 */
export function getUserBugReports(userId: string): BugReport[] {
  return db.prepare(`
    SELECT * FROM bug_reports
    WHERE user_id = ?
    ORDER BY created_at DESC
  `).all(userId) as BugReport[];
}
 ...
 */
export function countBugsByStatus(): Record<BugStatus, number> {
  const rows = db.prepare(`
    SELECT status, COUNT(*) as count
    FROM bug_reports
    GROUP BY status
  `).all() as { status: BugStatus; count: number }[];

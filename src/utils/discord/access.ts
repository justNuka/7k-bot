...
export function isInAllowedChannel(i: ChatInputCommandInteraction, allowed: ReadonlyArray<string>) {
  if (!i.inGuild() || allowed.length === 0) return true;
  return allowed.includes(i.channelId);
}

export async function hasOneOfRoles(i: ChatInputCommandInteraction, roles: ReadonlyArray<string>) {
  if (!i.inGuild() || roles.length === 0) return true;
...
export async function requireAccess(
  i: ChatInputCommandInteraction,
  {
    ...
    channels = [],
    reason = 'Accès refusé.'
  }: { roles?: ReadonlyArray<string>; channels?: ReadonlyArray<string>; reason?: string }
) {
...
export function botCanManageRole(i: ChatInputCommandInteraction, roleId?: string) {

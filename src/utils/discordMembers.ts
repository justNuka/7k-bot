// src/utils/discordMembers.ts
import type { Client, GuildMember } from 'discord.js';

export function avatarUrlFrom(member: GuildMember): string {
  const u = member.user;
  if (u.avatar) {
...
export async function fetchMembersSafe(client: Client, guildId: string, ids: string[]) {
  const g = await client.guilds.fetch(guildId);
  const out: Record<string, GuildMember | null> = {};

...
function getNextWeekday(from: Date, targetDay: number): Date {
  ...
  if (daysToAdd <= 0) daysToAdd += 7; // Si c'est aujourd'hui ou passé, on prend la semaine prochaine
 ...
 */
export function getDateSuggestions(focused: string): Array<{ name: string; value: string }> {
  const now = new Date();
  ...
  if (focused && focused.length > 0) {
    ...
    const filtered = quickOptions.filter(opt => 
      opt.name.toLowerCase().includes(lower) || opt.value.toLowerCase().includes(lower)
    );

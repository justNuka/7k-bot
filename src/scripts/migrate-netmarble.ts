...
type SeenStore = Record<NmCategoryKey, string[]>;
...
async function main() {
  ...
  try {
    ...
    const seen = await readJson<SeenStore>(JSON_PATH, { 
      notices: [], 
      updates: [], 
      known: [], 
      devnotes: [] 
    ...
    const articles: { category: NmCategoryKey; id: string; url: string }[] = [];
    
    // Convertir en format DB
    for (const cat of ['notices', 'updates', 'known', 'devnotes'] as NmCategoryKey[]) {
      const ids = seen[cat] || [];

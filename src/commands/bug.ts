...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption(o => o
    ...
    .setRequired(true)
    .addChoices(
      ...COMMANDS.map(cmd => ({ name: `/${cmd}`, value: cmd })),
      { name: '🔧 Autre (non lié à une commande)', value: 'autre' }
    )
  )
  .addStringOption(o => o
    .setName('description')
...
export async function execute(interaction: any) {
  ...
  try {
    ...
    const userEmbed = new EmbedBuilder()
      ...
      .addFields(
        { name: '🆔 Référence', value: `\`${bug.id}\``, inline: true },
        { name: '📝 Commande', value: command === 'autre' ? 'Autre' : `\`/${command}\``, inline: true },
        { name: '💬 Description', value: description.length > 100 ? description.slice(0, 100) + '...' : description }
      )
    ...
    if (notifChannel) {
      const officerEmbed = new EmbedBuilder()
        ...
        .addFields(
          { name: '🆔 ID', value: `\`${bug.id}\``, inline: true },
          { name: '📝 Commande', value: command === 'autre' ? 'Autre' : `\`/${command}\``, inline: true },

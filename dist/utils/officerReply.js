...
export async function officerDefer(i) {
 ...
 */
export async function officerReply(i, options) {
    const inside = inOfficerChannel(i);
 ...
 */
export async function officerEdit(i, options) {
    const inside = inOfficerChannel(i);
    if (typeof options === 'string') {
...
export async function officerDeferPublic(i) {
...
async function mirrorToOfficers(i, options) {
    ...
    if (MIRROR_PING_BLOCKLIST.has(key) || MIRROR_PING_BLOCKLIST.has(i.commandName ?? ''))
        return;
    // 2) Ping conditionnel
    ...
    if (typeof options === 'string') {
        ...
        await sendToChannel(i.client, CHANNEL_IDS.RETOURS_BOT, payload);
    }
}

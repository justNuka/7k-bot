...
const lastPostByUser = new Map(); // userId -> lastTimestamp
function makeJumpLink(gid, cid, mid) {
    return `https://discord.com/channels/${gid}/${cid}/${mid}`;
}
export async function onCandidatureMessage(msg) {
    try {
        if (msg.author.bot || msg.webhookId || !msg.guild)
            return;
        if (msg.channelId !== CHANNEL_IDS.CANDIDATURES)
            return;
        // ignore si officier
        const member = await msg.guild.members.fetch(msg.author.id).catch(() => null);
        if (!member)
            return;
        if (member.roles.cache.has(ROLE_IDS.OFFICIERS))
            return;
        // cooldown anti spam
        const now = Date.now();
        ...
        if (ROLE_IDS.RECRUES && !member.roles.cache.has(ROLE_IDS.RECRUES)) {
            try {
                ...
                roleNote = ` — ⚠️ Erreur roleswap: ${err.message}`;
            }
        }
        ...
        const emb = new EmbedBuilder()
            ...
            .addFields({ name: 'ID', value: `\`${msg.id}\``, inline: true }, { name: 'Lien', value: `[Ouvrir la candidature](${jumpLink})`, inline: true })
            .setFooter({ text: 'Ajoutée à la liste des candidatures ouvertes' })
            .setTimestamp();
        if (msg.content) {
            const preview = msg.content.trim().slice(0, 400);
            if (preview)
                emb.addFields({ name: 'Aperçu', value: preview });
        }

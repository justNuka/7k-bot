...
import { listSubs, getSubByChannel, insertSub, deleteSubByChannel, updateSubThread } from '../db/yt.js';
...
export async function execute(interaction) {
    ...
    try {
        ...
        if (sub === 'remove') {
            const channelId = interaction.options.getString('channel_id', true).trim();
            if (!getSubByChannel(channelId))
                return officerEdit(interaction, '❌ Chaîne introuvable.');
        ...
        if (sub === 'list') {
            const list = listSubs();
            if (!list.length)
                return officerEdit(interaction, 'Aucune chaîne suivie.');
        ...
        if (sub === 'test') {
            ...
            try {
                ...
                if (!last)
                    return officerEdit(interaction, 'Aucune vidéo trouvée sur le flux.');
                const chan = await interaction.client.channels.fetch(subRow.thread_id).catch(() => null);
                if (!chan || !chan.isTextBased())
                    return officerEdit(interaction, 'Thread introuvable/non textuel.');
                // @ts-ignore
                ...
                pushLog({ ts: new Date().toISOString(), level: 'error', component: 'yt',
                    ...
                    meta: { channelId, userId: interaction.user.id, error: e.message } });

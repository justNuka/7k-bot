...
import { loadNotifs, createNotif, editNotif, removeNotif } from '../jobs/notifs.js'; // ⬅️ MAJ imports (plus de saveNotifs ici)
...
export const data = new SlashCommandBuilder()
    ...
    .addChannelOption(o => o.setName('channel').setDescription('Salon où poster').addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement).setRequired(true))
    .addStringOption(o => o.setName('heure').setDescription('HH:MM (24h)').setRequired(true))
    .addStringOption(o => o.setName('freq').setDescription('Fréquence').setRequired(true).addChoices({ name: 'quotidien', value: 'daily' }, { name: 'jours ouvrés (lun–ven)', value: 'weekdays' }, { name: 'week-end (sam, dim)', value: 'weekends' }, { name: 'lundi', value: 'mon' }, { name: 'mardi', value: 'tue' }, { name: 'mercredi', value: 'wed' }, { name: 'jeudi', value: 'thu' }, { name: 'vendredi', value: 'fri' }, { name: 'samedi', value: 'sat' }, { name: 'dimanche', value: 'sun' }))
    ...
    .addChannelOption(o => o.setName('channel').setDescription('Nouveau salon (optionnel)').addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement))
    .addStringOption(o => o.setName('heure').setDescription('Nouvelle heure HH:MM (optionnel)'))
    .addStringOption(o => o.setName('freq').setDescription('Nouvelle fréquence (optionnel)').addChoices({ name: 'quotidien', value: 'daily' }, { name: 'jours ouvrés (lun–ven)', value: 'weekdays' }, { name: 'week-end (sam, dim)', value: 'weekends' }, { name: 'lundi', value: 'mon' }, { name: 'mardi', value: 'tue' }, { name: 'mercredi', value: 'wed' }, { name: 'jeudi', value: 'thu' }, { name: 'vendredi', value: 'fri' }, { name: 'samedi', value: 'sat' }, { name: 'dimanche', value: 'sun' }))
...
export async function execute(interaction) {
    ...
    try {
        ...
        if (sub === 'add') {
            ...
            const freq = interaction.options.getString('freq', true);
            const msg = interaction.options.getString('message') ?? '🔔 <@&ROLE> Rappel Castle Rush !';
            const spec = hhmmToSpec(heure, freq);
            if (!spec)
                return officerEdit(interaction, '❌ Heure invalide. Format attendu : `HH:MM` (24h).');
            const n = {
                id: newId(),
                roleId: role.id,
                channelId: chan.id,
        ...
        if (sub === 'list') {
            if (!list.length)
                return officerEdit(interaction, 'Aucune notification configurée.');
            const lines = list.map(n => `• **${n.id}** — <@&${n.roleId}> → <#${n.channelId}> — \`${n.spec}\` (${n.tz})`).join('\n');
            pushLog({
                ts: new Date().toISOString(),
                level: 'info',
                component: 'notif',

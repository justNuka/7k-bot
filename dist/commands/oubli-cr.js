...
async function isOfficer(i) {
    if (!i.inGuild() || !OFFICER_ROLE_ID)
        return false;
...
export const data = new SlashCommandBuilder()
    ...
    .addSubcommand((sc) => sc.setName('week').setDescription('Affiche le récap hebdomadaire (lun→dim)'))
    .addSubcommand((sc) => sc.setName('top').setDescription('Classement global des oublis (10 premiers)'))
    ...
    .addUserOption((o) => o
    .setName('membre')
    .setDescription('Membre à remettre à 0 (laisser vide pour tout le monde)')))
    .addSubcommand((sc) => sc.setName('week').setDescription('Reset du suivi hebdomadaire')));
export async function execute(interaction) {
    ...
    try {
        ...
        if (sub === 'add') {
            ...
            const already = db
                .prepare('SELECT 1 FROM cr_week WHERE week_start = ? AND day = ? AND user_id = ? LIMIT 1')
                .get(weekStart, jour, user.id);
            if (!already) {
                db.prepare('INSERT INTO cr_week(week_start, day, user_id) VALUES(?, ?, ?)').run(weekStart, jour, user.id);
            }
        ...
        if (sub === 'week' && !group) {
            const rows = db
                .prepare('SELECT day, user_id FROM cr_week WHERE week_start = ? ORDER BY day')
                .all(weekStart);
            const map = {
                mon: [],
                tue: [],
            ...
            for (const r of rows)
                map[r.day].push(r.user_id);
            const fields = ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'].map((k) => {
                const users = map[k];
        ...
        if (sub === 'top' && !group) {
            const rows = db
                .prepare('SELECT user_id, total FROM cr_counters ORDER BY total DESC LIMIT 10')
                .all();
            if (!rows.length)
                return crEdit(interaction, 'Aucun oubli enregistré.');
            const lines = rows.map((r) => `**${r.total}** — ${mentionOrName(interaction, r.user_id)}`);
            pushLog({
                ts: new Date().toISOString(),
                level: 'info',
                component: 'cr',
        ...
        if (group === 'reset' && sub === 'total') {
            ...
            const row = new ActionRowBuilder().addComponents(new ButtonBuilder()
                ...
                .setLabel('✅ Confirmer RESET global'), new ButtonBuilder()
        ...
        if (group === 'reset' && sub === 'week') {
            ...
            return crEdit(interaction, `🧹 Suivi hebdo réinitialisé (semaine du ${discordAbsolute(weekStart, 'f')}).`);
        }
    }
    catch (e) {
        ...
        pushLog({
            ...
            meta: { officerId: interaction.user.id, error: e.message },
        });
    }
}
function memberDisplay(member, fallback) {
    if (member?.nickname)

...
import { newCoachingId, createRequest, listOpenRequests, acceptRequest, closeRequest, getRequestById, listAcceptedBy } from '../db/coaching.js';
export const data = new SlashCommandBuilder()
    ...
    .setRequired(true)
    .addChoices({ name: 'Help rapide', value: 'help' }, { name: 'Coaching', value: 'coaching' }))
    .addStringOption(o => o
    .setName('message')
    ...
    try {
        ...
        if (sub === 'request') {
            ...
            if (retChanId) {
                ...
                try {
                    await sendToChannel(interaction.client, retChanId, content);
                }
                catch { }
            }
            pushLog({
        ...
        if (sub === 'list') {
            ...
            const lines = open.map(r => `• \`${r.id}\` — <@${r.user_id}> — **${r.type}** — ${new Date(r.created_at).toLocaleString('fr-FR')} \n> ${r.message}`).join('\n\n');
        ...
        if (sub === 'accept') {
            ...
            if (CHANNEL_IDS.RETOURS_BOT) {
                ...
                if (r) {
                    ...
                    try {
                        await sendToChannel(interaction.client, CHANNEL_IDS.RETOURS_BOT, msg);
                    }
                    catch { }
                }
            }
        ...
        if (sub === 'close') {
            ...
            if (CHANNEL_IDS.RETOURS_BOT) {
                ...
                try {
                    await sendToChannel(interaction.client, CHANNEL_IDS.RETOURS_BOT, txt);
                }
                catch { }
            }
            pushLog({
        ...
        if (sub === 'mine') {
            ...
            if (!mine.length)
                return officerEdit(interaction, 'Tu n’as aucune demande acceptée en cours.');
            const lines = mine.map(r => `• \`${r.id}\` — <@${r.user_id}> — **${r.type}** — accepté le ${new Date(r.accepted_at ?? '').toLocaleString('fr-FR')}\n> ${r.message}`).join('\n\n');
            return officerEdit(interaction, { embeds: [makeEmbed({
        ...
        pushLog({
            ...
            meta: { userId: interaction.user.id, error: e.message }
        });
        return;
    }
}

...
function targetCheck(i, target) {
    if (target.id === i.guild.ownerId)
        return 'On ne peut pas modifier le propriétaire du serveur.';
    return null;
}
...
export async function execute(interaction) {
    ...
    try {
        ...
        const member = await interaction.guild.members.fetch(interaction.options.getUser('membre', true).id);
        const block = targetCheck(interaction, member);
        if (block)
            return officerEdit(interaction, `❌ ${block}`);
        if (sub === 'swap') {
            ...
            if (member.roles.cache.has(from.id))
                await member.roles.remove(from, 'roleset swap');
            if (!member.roles.cache.has(to.id))
                await member.roles.add(to, 'roleset swap');
            pushLog({
                ts: new Date().toISOString(),
                level: 'action',
                component: 'roleset',
        ...
        pushLog({
            ...
            meta: { interactionId: interaction.id, error: e.message }
        });
        return;
    }
}

...
export async function execute(interaction) {
    ...
    try {
        ...
        if (sub === 'list') {
            ...
            const lines = items.map(r => `• \`${r.id}\` — <@${r.target_id}> — par <@${r.created_by}> — ${new Date(r.created_at).toLocaleString()}\n  ↳ ${r.note}`).join('\n');
            pushLog({
                ts: new Date().toISOString(),
                level: 'info',
                component: 'signalement',
        ...
        if (sub === 'remove') {
            ...
            if (!changes)
                return officerEdit(interaction, '❌ Rien supprimé (déjà supprimé ?)');
            pushLog({
                ts: new Date().toISOString(),
                level: 'info',
                component: 'signalement',

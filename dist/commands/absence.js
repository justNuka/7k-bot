...
export async function execute(interaction) {
    ...
    try {
        if (sub === 'add') {
            ...
            if (!debut)
                return interaction.editReply(`❌ Date de début invalide : "${debutInput}"\n💡 Formats acceptés : "demain", "lundi", "15/11/2025", "15 novembre", etc.`);
            if (!fin)
                return interaction.editReply(`❌ Date de fin invalide : "${finInput}"\n💡 Formats acceptés : "vendredi", "dans 5 jours", "20/11/2025", etc.`);
            const s = dayjs.tz(debut, 'YYYY-MM-DD', TZ);
            const e = dayjs.tz(fin, 'YYYY-MM-DD', TZ);
            if (!s.isValid() || !e.isValid())
                return interaction.editReply('❌ Dates invalides après parsing.');
            if (e.isBefore(s))
                return interaction.editReply('❌ La date de fin doit être **après ou égale** à la date de début.');
        ...
        if (sub === 'list') {
            ...
            if (!items.length)
                return officerEdit(interaction, 'Aucune absence en cours / à venir.');

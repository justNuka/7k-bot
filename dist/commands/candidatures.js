...
// DB
import { countOpenCandidatures, listOpenCandidaturesPaged, setCandidatureStatus, getCandidatureById, } from '../db/candidatures.js';
    ...
    try {
        ...
        for (const c of slice) {
            ...
            rows.push(new ActionRowBuilder().addComponents(new ButtonBuilder()
                ...
                .setLabel(`✅ ${sid}`), new ButtonBuilder()
        ...
        if (maxPage > 1) {
            rows.push(new ActionRowBuilder().addComponents(new ButtonBuilder()
                ...
                .setDisabled(cur <= 1), new ButtonBuilder()
                .setCustomId(`cand:page:${cur + 1}`)
                .setStyle(ButtonStyle.Secondary)
...
// ---------- Buttons handler ----------
export async function handleCandidaturesButton(interaction) {
    try {
        // officers-only
        const isOfficer = interaction.member && 'roles' in interaction.member
            ? interaction.member?.roles?.cache?.has?.(ROLE_IDS.OFFICIERS)
            : false;
        if (!isOfficer) {
            pushLog({
                ts: new Date().toISOString(),
        ...
        if (action === 'accept' || action === 'reject') {
            ...
            try {
                ...
                extraNote = ` — ⚠️ Erreur roleswap: ${err.message}`;
            }
            // log public

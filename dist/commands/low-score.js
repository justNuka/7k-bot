...
export async function execute(interaction) {
    ...
    try {
        ...
        if (sub === 'week') {
            ...
            const rows = db.prepare('SELECT day, user_id, score, note FROM low_week WHERE week_start = ? ORDER BY day').all(weekStart);
            const map = {
                mon: [], tue: [], wed: [], thu: [], fri: [], sat: [], sun: []
            };
            for (const r of rows)
                map[r.day].push({ userId: r.user_id, score: r.score, note: r.note ?? null });
        ...
        if (sub === 'reset') {
            ...
            return crEdit(interaction, `🧹 Low scores hebdo réinitialisés (semaine du ${discordAbsolute(weekStart, 'f')}).`);
        }
    }
    catch (e) {
